...
def gcs_folder_to_url(gcs_folder):
    """Reverse conversion: GCS folder name back to URL"""
    return f"https://{gcs_folder.replace('_', '/')}"
...
def get_gcs_image_links(bucket_name, gcs_folder):
    ...
    # Sort links to maintain correct page order
    image_links.sort()
...
def upload_to_gcs(bucket_name, image, destination_blob_name, dataset):
    ...
    temp_image_path = os.path.join(os.getcwd(), destination_blob_name.replace("/", "_"))  # Avoid subfolders
...
def load_pdf_and_upload(file_path, bucket_name, gcs_folder):
    ...
    return dataset  # Return the list of uploaded image links
...
@tool
def query_search_db(query: str, content_url, model, processor, qdrant_client):
    """
    ...
    Parameters:
            query:str
    Returns:
        str: The response from Gemini based on the search results
    """
...
def create_langchain_graph(tools, llm_with_tools):
    ...
    # Use builder.set_entry_point instead of START edge
    builder.set_entry_point("assistant")
    
    builder.add_conditional_edges(
        "assistant", tools_condition  # Route to tools or END based on assistant output
    )
...
def process(content_url, user_query, google_api_key, google_credentials_path):
    ...
    # Create a tool that directly embeds all needed context
    @tool
    def search_pdf(query: str) -> str:
        ...
        # Process directly inside the tool
        colqwen_query = batch_embed_query(query, processor, model)
        answer_colqwen = reranking_search_batch(colqwen_query, COLLECTION_NAME, content_url, qdrant_client)
        
        # Continue with the rest of what used to be in query_search_db
        top_10_results = []
        for point in answer_colqwen[0].points[:10]:
            top_10_results.append({"image_link": point.payload['image_link']})
